...
paf_file_t *paf_reader::paf_open(const char *fn)
{
	kstream_t *ks;
	gzFile fp;
	paf_file_t *pf;
...
int paf_reader::paf_close(paf_file_t *pf)
{
	kstream_t *ks;
	if (pf == 0) return 0;
	free(pf->buf.s);
...
int paf_reader::paf_parse(int l, char *s, paf_rec_t *pr) // s must be NULL terminated
{ // on return: <0 for failure; 0 for success; >0 for filtered
	...
	for (i = t = 0, q = s; i <= l; ++i) {
		...
		// else if ((t == 21 || t == 22 || t == 23) && (*q)=='c' && (*(q+1))=='s') pr->seq = q+5;
		else if (t == 11) pr->seq = q;
		++t, q = i < l? &s[i+1] : 0;
	}
	if (t < 10) return -1;
	return 0;
...
int paf_reader::paf_read(paf_file_t *pf, paf_rec_t *r)
{
	int ret, dret;
file_read_more:
