...
int clean_graph(asg_t *g, string inFileName, string outFileName, map<string,string>* excluded_nodes) {
    ...
    for(auto i : to_check){
        ...
        if(found_incoming && found_outgoing){
            ...
            for(int j = 0; j < num_incoming_arcs; j++){
                for(int i = 0; i < num_incoming_arcs; i++){
                    if((all_incoming_arcs[i].v>>1) != (all_incoming_arcs[j].v>>1) && unaccessable.find(all_incoming_arcs[j].v>>1) == unaccessable.end()
                    && unaccessable.find(all_incoming_arcs[i].v>>1) == unaccessable.end()){
            ...
            for(int j = 0; j < num_outgoing_arcs; j++){
                for(int i = 0; i < num_outgoing_arcs; i++){
                    if((all_outgoing_arcs[i].v>>1) != (all_outgoing_arcs[j].v>>1) && unaccessable.find(all_outgoing_arcs[j].v>>1) == unaccessable.end()
                    && unaccessable.find(all_outgoing_arcs[i].v>>1) == unaccessable.end()){
    ...
    for(auto c : new_connections){
        if(unaccessable.find(c.first>>1) == unaccessable.end() && unaccessable.find(c.second>>1) == unaccessable.end()){
            outfile << "L" << "\t" << g->seq[c.first>>1].name << "\t" << (c.first%2 == 0 ? "+" : "-") << "\t" << g->seq[c.second>>1].name << "\t" << (c.second%2 == 0 ? "+" : "-");
